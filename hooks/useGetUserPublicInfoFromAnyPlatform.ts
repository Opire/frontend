...
export const useGetUserPublicInfoFromAnyPlatform = ({ userId, revalidateOnFocus }: { userId: string, revalidateOnFocus?: boolean }) => {
    ...
    const usernameLink = getPlatformUserProfileLink({ platform: userPlatformToUse?.platform ?? 'GitHub', username });
