...
export function Filters() {
  const { populateParamToURL, populateMultipleParamToURL, searchParams } = usePopulateToURL();
  ...
  function getFiltersFromURL(): RewardFilters {
    ...
    const programmingLanguages = searchParams.get(URL_KEYS.PROGRAMMING_LANGUAGES)
      ? (searchParams.get(URL_KEYS.PROGRAMMING_LANGUAGES)!.split(',') as ProgrammingLanguageType[])
      : DEFAULT_REWARD_FILTERS.programmingLanguages;

    const usersTrying = searchParams.get(URL_KEYS.USERS_TRYING)
      ? (searchParams.get(URL_KEYS.USERS_TRYING)! as RewardFilterUserTrying)
  ...
  const updateProgrammingLanguageFilter = (programmingLanguages: ProgrammingLanguageType[]) => {
    ...
    populateParamToURL(URL_KEYS.PROGRAMMING_LANGUAGES, programmingLanguages.join(','));
  };
  ...
  const updateUsersTryingFilter = (usersTrying: RewardFilterUserTrying) => {
    ...
    populateParamToURL(URL_KEYS.USERS_TRYING, usersTrying === 'BOTH' ? '' : usersTrying);
  };
  ...
  return (
    ...
    >
      <PriceFilter value={[filters.price.min, filters.price.max]} onApply={updatePriceFilter} />
      <ProgrammingLanguagesFilter
        value={filters.programmingLanguages}
        onApply={updateProgrammingLanguageFilter}
      />

      <UsersTryingFilter value={filters.usersTrying} onApply={updateUsersTryingFilter} />
    </div>
  );
}
