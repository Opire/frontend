...
export const HomeRewardCard: FC<HomeRewardCardProps> = ({ data, inputRef }) => {
  ...
  const [usersToShow, usersHidden] = splitToShow(data.claimerUsers, MAX_NUMBER_OF_USERS_TO_SHOW);
  ...
  return (
    ...
    >
      ...
      <CardSection withBorder p="md">
        <Group justify="space-between" align="flex-end" style={{ height: '100%' }} wrap="nowrap">
            ...
            <Group gap="xs">
              <AvatarGroup>
                {usersToShow.map(user => (
                  <HoverCard withArrow shadow="md" closeDelay={0} openDelay={0} key={user.id}>
                    <HoverCardTarget>
                      <Avatar src={user.avatarURL} alt={user.username} size="md" radius="xl" />
                    </HoverCardTarget>

                    <HoverCardDropdown>{user.username}</HoverCardDropdown>
                  </HoverCard>
                ))}
                {hasMoreUsers && (
