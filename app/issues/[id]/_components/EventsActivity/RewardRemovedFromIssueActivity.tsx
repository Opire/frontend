...
export const RewardRemovedFromIssueActivity: FC<RewardRemovedFromIssueActivityProps> = ({
  event,
}) => {
  ...
  const { isLoading, username, avatarURL, usernameLink } = useGetUserPublicInfoFromPlatform({
    userId: attributes.reward.creatorId,
    platform,
  });
    ...
    >
      ...
      {!isLoading && (
        <Text c="dimmed" size="sm">
          <Link href={usernameLink}>@{username}</Link> deleted a{' '}
          <Text variant="gradient" component="span" style={{ fontWeight: 'bold' }}>
            {formatPrice(attributes.reward.price)}
          </Text>{' '}
          reward
        </Text>
      )}
