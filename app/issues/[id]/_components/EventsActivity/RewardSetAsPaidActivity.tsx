...
export const RewardSetAsPaidActivity: FC<RewardSetAsPaidActivityProps> = ({ event }) => {
    const attributes = event.attributes as { rewards: RewardPrimitive[], issue: IssuePrimitive, rewardsTotalPrice: PricePrimitive, creatorId: string };
    ...
    return (
        <Timeline.Item title="Reward paid" bullet={
            <AvatarGroup>
                <Avatar
                    src={rewardCreatorAvatarURL}
                    alt={rewardCreatorUsername}
                    size='sm'
                    radius='xl'
                />
                <Avatar
                    src={rewardedUserAvatarURL}
                    alt={rewardedUserUsername}
                    size='sm'
                    radius='xl'
                />
            </AvatarGroup>
            ...
            {!isLoading &&
                <Text c="dimmed" size="sm">
                    <Link href={rewardCreatorUsernameLink}>@{rewardCreatorUsername}</Link> paid <Text variant="gradient" component="span" style={{ fontWeight: "bold" }}>{formatPrice(attributes.rewardsTotalPrice)}</Text> in rewards to <Link href={rewardedUserUsernameLink}>@{rewardedUserUsername}</Link>
                </Text>
            }
