...
export const UserClaimedActivity: FC<UserClaimedActivityProps> = ({ event }) => {
  ...
  const { isLoading, username, usernameLink, avatarURL } = useGetUserPublicInfoFromPlatform({
    ...
    >
      ...
      {!isLoading && (
        <Text c="dimmed" size="sm">
          <Link href={usernameLink}>@{username}</Link> has proposed a solution for the issue and is
