...
export const DetailedIssueMainData: FC<DetailedIssueMainDataProps> = ({ issue, userAuth }) => {
  ...
  const [isAddRewardModalOpen, { close: closeAddRewardModal, open: openAddRewardModal }] =
    useDisclosure();
  const [isClaimRewardsModalOpen, { close: closeClaimRewardsModal, open: openClaimRewardsModal }] =
    useDisclosure();

  useTriggerCallbackOnQueryParamFirstMatch({
    queryParamKey: 'add-reward',
    callback: openAddRewardModal,
  });
  useTriggerCallbackOnQueryParamFirstMatch({
    queryParamKey: 'claim-rewards',
    callback: openClaimRewardsModal,
  });
  ...
  const availablePrice: PricePrimitive = issue.rewards.reduce(
    (acc, el) => {
  ...
  const labelsAndProgrammingLanguagesToShow = [
    ...issue.labels.filter(label => !label.includes('Reward')),
  ...
  return (
    <section style={{ height: 'auto' }}>
        ...
        <Grid.Col span={{ base: 12, md: 5, lg: 4 }}>
          ...
          <Center>
            <Flex direction={'column'}>
              <Text style={{ fontSize: '2.4rem', fontWeight: 'bold' }} variant="gradient">
                {formatPrice(totalPrice)}
              </Text>

              {somethingHasBeenAlreadyPaid && (
                <Flex align={'center'} columnGap={'0.4rem'} wrap={'wrap'}>
                  <Text>Available:</Text>

                  <Text style={{ fontSize: '1.4rem', fontWeight: 'bold' }} variant="gradient">
      ...
      <Card withBorder shadow="md" radius="md">
        <Center>
          <Text style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>Rewards</Text>
        </Center>
        ...
        <Table.ScrollContainer minWidth={500}>
          <Table verticalSpacing="md" highlightOnHover>
            ...
            <Table.Tbody>
              {issue.rewards
                .sort((reward1, reward2) => reward2.createdAt - reward1.createdAt)
                .map(reward => (
...
const DetailedIssueReward: FC<{ reward: RewardPrimitive; issue: IssuePrimitive }> = ({
  reward,
  issue,
}) => {
  const platform = issue.platform;

  const { isLoading, username, avatarURL, usernameLink } = useGetUserPublicInfoFromPlatform({
  ...
  return (
    <>
      <Table.Td>{new Intl.DateTimeFormat('en-GB').format(new Date(reward.createdAt))}</Table.Td>
      ...
      <Table.Td>
        <Badge color={reward.status === 'Available' ? 'teal' : 'blue'}>{reward.status}</Badge>
      </Table.Td>
...
const UserTryingCard: FC<{ userId: string; platform: PlatformType; hasClaimed: boolean }> = ({
  userId,
  platform,
  hasClaimed,
}) => {
