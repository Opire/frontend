...
export const Navbar: FC<NavbarProps> = ({ userAuth }) => {
  ...
  return (
    <>
      ...
      <Divider my="sm" />
      <NavbarMenu menuItems={menuItems} path={path} userAuth={userAuth} />
...
function NavbarMenu({
  ...
  return (
    <>
      {menuItems
        .filter(item => item.isPublic || userAuth !== null)
        .map(item => (
          <NavbarMenuItem key={item.id} item={item} path={path} userAuth={userAuth} />
        ))}
    </>
  );
}
...
function NavbarMenuItem({
    ...
    <>
      {
        ...
        >
          {item.children && item.children.length > 0 && (
            <NavbarMenu menuItems={item.children} userAuth={userAuth} path={path} />
          )}
        </NavLink>
      }
    </>
  ...
  function shouldStartOpened(item: ItemMenu, path: string): boolean {
    ...
    const childrenHaveSamePath = item.children?.some(
      child => child.path === path || shouldStartOpened(child, path)
    );
