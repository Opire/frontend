...
export function getChallengePrizeMaxPosition(
  primitive: ChallengePrizePrimitive
): number | CHALLENGE_PRIZE_WITHOUT_LIMIT {
  if (isPrimitiveSpecificPositionPrize(primitive)) {
    return primitive.position;
  }
...
export function isPrimitiveSpecificPositionPrize(
  primitive: ChallengePrizePrimitive
): primitive is SpecificPositionPrizePrimitive {
  return 'position' in primitive && !Number.isNaN(+primitive.position);
}

export function isPrimitiveThresholdPrize(
  primitive: ChallengePrizePrimitive
): primitive is ThresholdPrizePrimitive {
  return (
    'fromPosition' in primitive &&
    !Number.isNaN(+primitive.fromPosition) &&
    'toPosition' in primitive &&
    primitive.toPosition !== CHALLENGE_PRIZE_WITHOUT_LIMIT_VALUE &&
    !Number.isNaN(+primitive.toPosition)
  );
}

export function isPrimitiveThresholdWithoutLimitPrize(
  primitive: ChallengePrizePrimitive
): primitive is ThresholdWithoutLimitPrizePrimitive {
  return (
    'fromPosition' in primitive &&
    !Number.isNaN(+primitive.fromPosition) &&
    'toPosition' in primitive &&
