...
export function getChallengeHighestPrize(challenge: ChallengePrimitive): PricePrimitive {
  ...
  const prizesSum: PricePrimitive = challenge.configuration.prizes.reduce(
    (accumulator: PricePrimitive, currentPrize: ChallengePrizePrimitive): PricePrimitive => ({
      value:
        accumulator.value + getPriceInUSD_CENT(currentPrize.amount ?? EMPTY_CHALLENGE_PRIZE_AMOUNT),
