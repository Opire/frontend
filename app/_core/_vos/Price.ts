...
export class Price {
    ...
    constructor (private _value: PositiveNumber, private _unit: PriceUnit) {
    ...
    public valueAs (priceUnit: PriceUnit): number {
        const result =
            this._value.value /
            PriceUnit.ConversionRateToUsdCents[priceUnit.value];
    ...
    public isBetween (lowerPrice: Price, upperPrice: Price): boolean {
        return (
            this.isEqualOrGreaterThan(lowerPrice) &&
            this.isEqualOrLowerThan(upperPrice)
        );
    }

    public static sum (prices: Price[]): Price {
        const pricesInUsdCent = prices.map((price) => price.valueAs(PriceUnit.USD_CENT));
        const totalPricesInUsdCent = pricesInUsdCent.reduce(
            (acc, number) => acc + number,
            0,
        );

        return new Price(
            new PositiveNumber(totalPricesInUsdCent),
            PriceUnit.USD_CENT,
        );
    }
