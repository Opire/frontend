...
import { CreateChallengeTemplate } from '../../../../../../hooks/useGetCreateChallengeTemplates';
import {
  ActionIcon,
  Affix,
  Alert,
...
export const DraftChallengeCreatorData: FC<DraftChallengeCreatorDataProps> = ({
  ...
  const form = useForm<EditDraftChallengeDTO>({
    ...
    onValuesChange: values => {
      ...
      debouncedOnUpdateDraft(challenge.id, values, reloadChallenge);
    },
  });
    ...
    <>
      <section style={{ height: 'auto' }}>
          ...
          {!isUpdatingDraft && (
            <Tooltip
              label={`Updated at ${formatDateTime(
                challenge ? new Date(challenge.updatedAt) : new Date()
              )}`}
            >
        ...
        <Box style={{ padding: '1rem 2rem' }}>
          ...
          <Alert variant="light" color="blue" title="Public content" icon={<IconInfoCircle />}>
            <span>
              The content you fill in the following form{' '}
              <strong>will be visible for everyone</strong> once the challenge is published.
            </span>
            ...
            <p>
              ...
              , while ensuring you provide clear and detailed information in each of the sections.
            </p>
          </Alert>
