...
export function PublishChallengeForm({
  ...
  isDisabled,
  isLoading,
}: PublishChallengeFormProps): React.ReactElement {
  ...
  async function publishChallenge(allowParticipationsAfterPublish: boolean) {
    try {
      ...
      await clientCustomFetch(API_ROUTES.CHALLENGES.PUBLISH_DRAFT(challengeId), {
        method: 'POST',
      });

      if (allowParticipationsAfterPublish) {
        await clientCustomFetch(API_ROUTES.CHALLENGES.ALLOW_NEW_PARTICIPATIONS(challengeId), {
          method: 'POST',
        });
      }
      ...
      >
        <Container size={'lg'}>
          ...
          >
            ...
            <Text>
              <strong>This cannot be undone</strong>, so make sure that you are happy with the
              challenge configuration before continue. Once published,{' '}
              <strong>you will not be able to edit your challenge</strong>.
            </Text>

            <Space h="1rem" />

            <Text>
              You only will be able to{' '}
              <strong>
                increase the prizes, increase the limit of participations, and extend the deadline
              </strong>
              . The rest of the content will not be editable anymore.
            </Text>
          </Alert>

          <Space h="1rem" />
          <Text>You can publish your challenge in two ways:</Text>
          <Space h="1rem" />

          <Title size={'h4'}>With open participations</Title>
          <Text>
            The challenge will be published, and the participants will be able to submit their
            solutions right away.
          </Text>
          <Space h="1rem" />
          <Text>
            This is ideal if you are ready to start receiving the solutions, and do not need to wait
            before the participants start to work in their solutions.
          </Text>
          ...
          <Space h="1rem" />
          <Text>
            This can give you some time to promote the challenge, share it in social media, or
            arrange any setup before start receiving solutions.
          </Text>
          ...
          <Text>
            Once the challenge is published, you will be able to close/open the participations at
            will as many times as you need, so don&apos;t worry if you are not sure which option to
            pick right now.
          </Text>
          ...
          >
            <Button onClick={closePublishChallengeModal} variant="subtle" size="md">
              Go back
            </Button>
