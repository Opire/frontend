...
export const ApproveChallengeParticipationModal: FC<ApproveChallengeParticipationModalProps> = ({
  challenge,
  participation,
  isOpened,
  onClose,
  ...
  const [isApprovingParticipation, setIsApprovingParticipation] = useState(false);
  ...
  async function approve() {
    try {
      ...
      await clientCustomFetch(
        API_ROUTES.CHALLENGES.APPROVE_PARTICIPATION({
  ...
  return (
    <Modal
      ...
      opened={isOpened}
      onClose={onClose}
      title={<Title size={'h3'}>Approve solution</Title>}
      size={'xl'}
      closeOnEscape={true}
    ...
    >
      <Container size={'lg'}>
        <Text>
          If you continue, the solution will be approved. This mean that the solution has been
          considered successful based on the evaluation criteria of the challenge.
        </Text>
