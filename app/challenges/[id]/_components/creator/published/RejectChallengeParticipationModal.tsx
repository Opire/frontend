...
export const RejectChallengeParticipationModal: FC<RejectChallengeParticipationModalProps> = ({
  challenge,
  participation,
  isOpened,
  onClose,
  ...
  const [isRejectingParticipation, setIsRejectingParticipation] = useState(false);
  ...
  async function reject({ reasonForRejection }: { reasonForRejection: string }) {
    try {
      ...
      await clientCustomFetch(
        API_ROUTES.CHALLENGES.REJECT_PARTICIPATION({
  ...
  return (
    <Modal
      ...
      opened={isOpened}
      onClose={onClose}
      title={<Title size={'h3'}>Reject solution</Title>}
      size={'xl'}
      closeOnEscape={true}
    ...
    >
      <Container size={'lg'}>
        <Text>
          If you continue, the participation will be rejected. This mean that the solution has not
          been considered successful based on the evaluation criteria of the challenge.
        </Text>

        <Space h="1rem" />

        <Text>
          Please, provide thoughtful feedback and describe in detail what&apos;s the reason for the
          rejection. This will be sent to the participant, so it&apos;s possible that they will
          submit another solution based on the feedback you provide.
        </Text>
