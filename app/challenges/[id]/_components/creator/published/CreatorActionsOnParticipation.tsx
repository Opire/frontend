...
export const CreatorActionsOnParticipation: FC<CreatorActionsOnParticipationProps> = ({
  challenge,
  participation,
}) => {
  const router = useRouter();
  const [isModalForApproveOpen, { close: closeModalForApprove, open: openModalForApprove }] =
    useDisclosure();
  const [isModalForPayOpen, { close: closeModalForPay, open: openModalForPay }] = useDisclosure();
  const [isModalForRejectOpen, { close: closeModalForReject, open: openModalForReject }] =
    useDisclosure();
  ...
  const { canApprove, canReject, canPay } = useMemo(() => {
    ...
    const canReject =
      participation.status === 'waiting_for_approval' || participation.status === 'approved';
          ...
          <Tooltip
            label={
              challenge.canPrizesBePaid
                ? 'Pay prize'
                : 'The challenge needs to be completed before you can pay the participant'
            }
          >
            <ActionIcon
              size="md"
              variant="light"
              color="green"
              disabled={!challenge.canPrizesBePaid}
              onClick={openModalForPay}
            >
              <IconTrophyFilled />
            </ActionIcon>
          </Tooltip>
