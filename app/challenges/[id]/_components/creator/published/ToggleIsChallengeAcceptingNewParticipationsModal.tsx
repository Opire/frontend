...
export const ToggleIsChallengeAcceptingNewParticipationsModal: FC<
  ToggleIsChallengeAcceptingNewParticipationsModalProps
> = ({ challenge, isOpened, onClose, onChallengeUpdated = () => {} }) => {
  ...
  async function updateChallenge() {
    try {
      ...
      notifications.show({
        title: isGoingToStopNewParticipations
          ? 'Participations stopped'
          : 'New participations allowed',
        message: isGoingToStopNewParticipations
          ? "No more participants will be able to submit their solution. Don't forget to re-allow new participations once you're ready to start receiving solutions for your challenge"
          : 'Now any participant will be able to submit their solution to the challenge 🎉',
        withBorder: true,
  ...
  return (
    <Modal
      ...
      title={
        <Title size={'h3'}>
          {isGoingToStopNewParticipations
            ? 'Disallow new participations'
            : 'Allow new participations'}
        </Title>
      }
      size={'xl'}
      closeOnEscape={true}
    ...
    >
      <Container size={'lg'}>
        ...
        <Group justify="space-between" mt="md">
          ...
          >
            {isGoingToStopNewParticipations
              ? 'Stop receiving participations'
              : 'Start receiving participations'}
          </Button>
        </Group>
      </Container>
    </Modal>
