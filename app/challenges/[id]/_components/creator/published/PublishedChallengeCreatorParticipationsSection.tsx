...
import {
  ChallengeParticipationPrimitive,
  ChallengeParticipationStatusType,
} from '../../../../../_core/_primitives/ChallengeParticipationPrimitive';
...
export const PublishedChallengeCreatorParticipationsSection: FC<{
  ...
  const participationsToShow = challenge.isCompleted
    ? challenge.participations.filter(participation => participation.status !== 'paid')
          ...
          <>
            <Center>
              <Alert
                ...
                icon={<IconInfoCircle />}
              >
                <Text>You are not accepting new participations at the moment.</Text>
            ...
            {challenge.isAcceptingParticipations && (
              <Center>
                <Alert
                  ...
                  icon={<IconInfoCircle />}
                >
                  <Text>No one has send their solution yet.</Text>

                  <Text>
                    If you want to attract the attention of the participants, you can try to share
                    the challenge in your social media.
                  </Text>

                  <br />

                  <Text>
                    If some time passes without a single participant, maybe your challenge is too
                    hard or the prizes are not high enough.
                  </Text>

                  <Text>
                    You may need to increase the prizes (you&apos;ll need to unpublish the challenge
                    first) or contact some potential sponsors.
                  </Text>
                </Alert>
              </Center>
            )}
...
const ParticipationRow: FC<{
  ...
  const { isLoading, username, avatarURL, usernameLink } = useGetUserPublicInfoFromAnyPlatform({
    ...
    <>
      ...
      <Table.Td miw={'180px'}>
        {participation.status === 'rejected' ? (
          <Tooltip
            ...
            label={
              <div>
                ...
                <Space h={'0.5rem'} />
                <q style={{ whiteSpace: 'pre-wrap' }}>{participation.reasonForRejection ?? ''}</q>
              </div>
            }
          >
            <Badge
          ...
          <Badge variant="light" color={PARTICIPATION_STATUS_COLOR[participation.status]}>
      ...
      <Table.Td>{formatDateTime(new Date(participation.createdAt))}</Table.Td>
