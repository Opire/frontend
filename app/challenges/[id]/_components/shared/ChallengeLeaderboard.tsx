...
export const ChallengeLeaderboard: FC<{
  ...
  const paidParticipations = challenge.participations.filter(
    participation => participation.status === 'paid'
  );
  const sortedPaidParticipations = [...paidParticipations].sort(
          ...
          <Table verticalSpacing="md" highlightOnHover>
            ...
            <Table.Tbody>
              ...
              {sortedPaidParticipations.length === 0 && (
                <Table.Tr>
                  <Table.Td colSpan={5}>
                    <Text ta="center">No winners yet</Text>
                  </Table.Td>
                </Table.Tr>
              )}
            </Table.Tbody>
...
const LeaderboardRow: FC<{
  ...
  return (
    <>
      <Table.Td>{getOrdinalPositionDescription(participation.position ?? 1)}</Table.Td>
      ...
      <Table.Td>
        <Text variant="gradient" style={{ fontWeight: 'bold', fontSize: '1.2rem' }}>
          {participation.prize && <PrizeDisplay prize={participation.prize} />}
        </Text>
      </Table.Td>
