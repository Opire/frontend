...
export const TipCard: FC<OwnerRewardCardProps> = ({ data, inputRef }) => {
    ...
    <Card ref={inputRef} withBorder shadow="md" radius="md">
      <Group justify="center">
        <Avatar
          ...
          size="lg"
        />
        <Text style={{ fontSize: '40px' }} fw={700} size="xl" variant="gradient">
          {formatPrice(data.price)}
        </Text>
      </Group>
      <Space h="20px" />
      <Group justify="space-between">
        <Button component="a" variant="subtle" target="_blank" href={data.commentURL}>
          Go to issue
        </Button>
        <Button variant="gradient" onClick={() => completePayTip(data.id)} disabled={!canBePaid}>
          {canBePaid ? 'Pay' : 'Paid'}
        </Button>
      </Group>
    </Card>
