...
export const CreatorRewardPaidCard: FC<CreatorRewardPaidCardProps> = ({ data, inputRef }) => {
  const totalIssueRewardPrice = Price.sum([
    Price.fromPrimitives(data.alreadyPaid),
    Price.fromPrimitives(data.pendingToBePaid),
  ]);
    ...
    >
      ...
      <CardSection withBorder inheritPadding p="md" style={{ minHeight: '88px', height: '100%' }}>
        <Group justify="space-between" align="flex-end" style={{ height: '100%' }}>
          <AvatarGroup>
            {data.usersTrying
              .filter(userTrying => userTrying.alreadyPaid.value > 0)
              .map(userTrying => (
                <>
                  <HoverCard withArrow shadow="md" closeDelay={0} openDelay={0} key={userTrying.id}>
                    <HoverCardTarget>
                      <Avatar
                        src={userTrying.avatarURL}
                        alt={userTrying.username}
                        color="teal"
                        size="lg"
                        radius="xl"
                      />
                    ...
                    <HoverCardDropdown>
                      <Text variant="default">Paid {formatPrice(userTrying.alreadyPaid)}</Text>
                    </HoverCardDropdown>
                  </HoverCard>
                </>
              ))}
