...
export const CreatorRewardUnpaidCard: FC<CreatorRewardUnpaidCardProps> = ({ data, inputRef }) => {
  ...
  const [
    ...
    { open: openModalForSelectUserToPay, close: closeModalForSelectUserToPay },
  ] = useDisclosure(false);
  ...
  const canRewardsBeRemoved = claimerUsers.length === 0 && data.alreadyPaid.value === 0;
  ...
  async function removeRewards() {
    ...
    try {
      await clientCustomFetch(API_ROUTES.REWARDS.DELETE_ALL_FROM_CREATOR(), {
        method: 'DELETE',
        body: {
          issue: {
            id: data.issueId,
  ...
  return (
    <>
      ...
      >
        ...
        <CardSection p="sm">
          <Group justify="space-between">
            <Text component="div" size={'xs'} style={{ textAlign: 'center' }}>
                  ...
                  {claimerUsersToShow.map(claimerUser => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <HoverCardDropdown>
                          <ClaimerUserCard user={claimerUser} paidPrice={totalPaidToUser} />
                        </HoverCardDropdown>
                      </HoverCard>
                    );
                  })}
                    ...
                    <HoverCard withArrow shadow="md" closeDelay={0} openDelay={0}>
                      ...
                      <HoverCardDropdown>
                        {claimerUsersHidden.map(claimerUser => (
                          <div key={claimerUser.id}>{claimerUser.username}</div>
                        ))}
                      </HoverCardDropdown>
                    </HoverCard>
                  )}
      ...
      >
        <section>
          <Text>
            Are you sure you want to remove all the rewards that you created for this issue?
          </Text>

          <Space h={'1rem'} />
          <Text>
            This will deduct the amount of <strong>{formatPrice(data.pendingToBePaid)}</strong> from
            the issue, reducing the possibility of being solved by a developer.
          </Text>
        </section>
        ...
        <Flex>
          <Button m="0 auto" variant="outline" onClick={closeModalForRemoveRewards}>
            Go back
          </Button>
